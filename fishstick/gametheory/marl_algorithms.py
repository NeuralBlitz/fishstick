...
class QLearning(MultiAgentRLAlgorithm):
    ...
    def update(
        ...
        for agent_id in range(self.n_agents):
            ...
            target_q = rewards.get(agent_id, 0.0) + self.gamma * max_next_q * (1 - done)
...
class PolicyGradientMARL(MultiAgentRLAlgorithm):
    ...
    def __init__(
        ...
        for agent_id in range(n_agents):
            ...
            self.policy_optimizers[agent_id] = torch.optim.Adam(
                self.policy_networks[agent_id].parameters(), lr=learning_rate
            )
            self.value_optimizers[agent_id] = torch.optim.Adam(
                self.value_networks[agent_id].parameters(), lr=learning_rate
            )
    ...
    def update(
        ...
        for agent_id in range(self.n_agents):
            ...
            value_loss = F.mse_loss(value, torch.tensor([rewards.get(agent_id, 0.0)]))
...
@dataclass
class NashQLearning(MultiAgentRLAlgorithm):
    ...
    def __init__(
        ...
        self.q_tables = {
            agent_id: np.zeros((100, act_dim)) for agent_id in range(n_agents)
        }
...
class MeanFieldQ(MultiAgentRLAlgorithm):
    ...
    def __init__(
        ...
        self.q_tables = {agent_id: np.zeros(act_dim) for agent_id in range(n_agents)}
    ...
    def update(
        ...
        for agent_id in range(self.n_agents):
            ...
            mean_field_q = np.sum(mean_action_probs * self.q_tables[agent_id])

            target_q = rewards.get(agent_id, 0.0) + self.gamma * mean_field_q * (
                1 - done
            )
...
def train_marl_agent(
    ...
    """
    episode_rewards: Dict[int, List[float]] = {i: [] for i in range(algorithm.n_agents)}
        ...
        for step in range(max_steps):
            ...
            for agent_id in range(algorithm.n_agents):
                action = algorithm.select_action(
                    state.observation, agent_id, explore=(episode < num_episodes * 0.8)
                )
