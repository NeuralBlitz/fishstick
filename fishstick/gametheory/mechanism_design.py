...
class Mechanism(ABC):
    ...
    def compute_payments(
        self, preferences: Dict[int, Preference], allocation: Allocation
    ) -> Dict[int, float]:
        """Compute payments for each agent."""
        pass

    def execute(self, preferences: Dict[int, Preference]) -> Allocation:
...
class MechanismDesign:
    ...
    def set_true_preferences(self, preferences: Dict[int, Preference]) -> None:
        """Set the true preferences of agents."""
        self.true_preferences = preferences

    def report_preferences(self, reports: Dict[int, Preference]) -> Allocation:
        """Process reported preferences."""
        self.reported_preferences = reports
        return None

    def is_truthful(self, mechanism: Mechanism) -> bool:
        ...
        for agent in self.agents:
            ...
            true_utility = true_pref.net_value(
                true_allocation.get_item(agent), true_allocation.get_payment(agent)
            )
    ...
    def is_efficient(self, mechanism: Mechanism) -> bool:
...
class VCGMechanism(Mechanism):
    ...
    def allocate(self, preferences: Dict[int, Preference]) -> Allocation:
        ...
        if len(self.items) == 1:
            best_agent = max(
                self.agents, key=lambda a: preferences[a].value(list(self.items)[0])
            )
            return Allocation(
        ...
        if len(item_list) == len(agent_list):
            ...
            agent_items = {
                agent_list[i]: item_list[j] for i, j in zip(row_ind, col_ind)
            }
    ...
    def compute_payments(
        ...
        for agent in self.agents:
            ...
            without_agent = {a: pref for a, pref in preferences.items() if a != agent}
            ...
            payments[agent] = (
                welfare_without
                - welfare_with
                + preferences[agent].value(allocated_item)
            )

        return payments

    def _max_social_welfare(self, preferences: Dict[int, Preference]) -> float:
        ...
        for agent in agent_list:
            best_value = max(preferences[agent].value(item) for item in self.items)
            total += best_value
...
class MyersonMechanism(Mechanism):
    ...
    social_choice_function: Callable[[Dict[int, Preference]], Any]
...
class UniformPriceMechanism(AuctionMechanism):
    ...
    def allocate(self, preferences: Dict[int, Preference]) -> Allocation:
        bids = [(agent, preferences[agent].value(self.item)) for agent in self.agents]
    ...
    def compute_payments(
        ...
        other_bids = [
            preferences[a].value(self.item) for a in self.agents if a != winner
        ]
...
def create_knapsack_mechanism(
    ...
    def allocate(prefs: Dict[int, Preference]) -> Allocation:
        ...
        item_values = [
            (item, sum(values[a](item) for a in agent_list)) for item in items
        ]
